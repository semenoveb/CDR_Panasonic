...
class CollectFiles:
    ...
    def __init__(self, folder_name, general_file):
        ...
        self.general_file = Path(folder_name, general_file)
...
def clear_file(file_read, file_write):
    header = ["Start_Time", "Duration_of_Call 'seconds'", "Called_Party",
              "Calling_Party", "Call_Answer_Time", "Call_Release_Time",
    ...
    try:
        with open(file_read) as reader, open(file_write, 'w') as writer:
            ...
            for line in reader:
                ...
                new_line = ','.join([line_l[2].split('.')[0], str(int(line_l[3]) / 10), line_l[10],
                                     line_l[11], line_l[47].split('.')[0], line_l[48].split('.')[0],
                                     line_l[64], line_l[125], line_l[126], '\n'])
...
def merge_files(src_file_name, tenants_file_name):
    try:
        finished_file = input("Enter merged file name with extension 'xlsx': ")
        ...
        clear_table.drop(columns=['Unnamed: 9'], inplace=True)
        all_tenants = pd.read_excel(tenants_file_name)
